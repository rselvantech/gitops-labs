...
```bash
kubectl get namespace sync-waves-demo
```
...
```
11-sync-waves/src/
├── app-config/              ← git init → remote: rselvantech/app-config (new private repo)
│   └── demo-11-sync-waves/
│       ├── namespace.yaml
│       ├── resourcequota.yaml
│       ├── limitrange.yaml
...
```
rselvantech/app-config (GitHub) — new private repo
└── demo-11-sync-waves/
...
> Only `app-config` needs to be registered with ArgoCD — it is the source
> repo ArgoCD clones at sync time. `argocd-config` is applied manually with
> `kubectl apply` and does not need ArgoCD registration.

---

## Step 1: Setup — Create Repo and Register with ArgoCD

### Step 1a: Create `app-config` private repo on GitHub
...
### Step 1c: Register `app-config` with ArgoCD

`app-config` is a new private repo — ArgoCD needs credentials to clone it:
...
**Expected:** `app-config` listed with `Successful` status.

If not listed, check the PAT has `Contents: Read-only` permission for
...
Create all files under `demo-11-sync-waves/`. The `argocd.argoproj.io/sync-wave`
annotation is the only addition beyond standard Kubernetes manifests.

**`demo-11-sync-waves/namespace.yaml`** — Wave -1
```yaml
apiVersion: v1
kind: Namespace
metadata:
...
**`demo-11-sync-waves/resourcequota.yaml`** — Wave -1
```yaml
apiVersion: v1
kind: ResourceQuota
metadata:
...
**`demo-11-sync-waves/limitrange.yaml`** — Wave -1
```yaml
apiVersion: v1
kind: LimitRange
metadata:
...
**`demo-11-sync-waves/serviceaccount.yaml`** — Wave -1
```yaml
apiVersion: v1
kind: ServiceAccount
metadata:
...
**`demo-11-sync-waves/secret.yaml`** — Wave -1
```yaml
apiVersion: v1
kind: Secret
metadata:
...
**`demo-11-sync-waves/configmap.yaml`** — Wave 0
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
**`demo-11-sync-waves/networkpolicy.yaml`** — Wave 0
```yaml
...
**`demo-11-sync-waves/service.yaml`** — Wave 1
```yaml
apiVersion: v1
kind: Service
metadata:
...
**`demo-11-sync-waves/deployment.yaml`** — Wave 5
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
**Push all manifests to `app-config`:**
```bash
git add demo-11-sync-waves/
git commit -m "feat: add demo-11 sync waves manifests with wave annotations"
...
> **Why push to Git AND `kubectl apply`?** ArgoCD watches `app-config` (the
...
```bash
kubectl get all,resourcequota,limitrange,networkpolicy,configmap,serviceaccount,secret \
  -n sync-waves-demo -w
```
...
**Create `demo-11-sync-waves/poddisruptionbudget.yaml`** — Wave 3:
```yaml
...
**Push:**
```bash
git add demo-11-sync-waves/poddisruptionbudget.yaml
...
# app-config registered with ArgoCD
argocd repo list
...
# Deregister app-config from ArgoCD
argocd repo rm https://github.com/rselvantech/app-config.git
```

> The `app-config` GitHub repo is reused in future demos — do not delete it
> from GitHub. Only deregister it from ArgoCD if you are done with the course.
...
# View all resources with wave annotations
kubectl get all -n sync-waves-demo \
...
**2. Wave -1 is the natural home for bootstrapping resources**
Any resource that must exist before everything else belongs in a negative wave.
Namespace, ResourceQuota, LimitRange, ServiceAccount, and Secret are the
standard wave -1 candidates. Wave 0 is the default for unannotated resources.

**3. Always leave gaps between wave numbers**
Production wave assignments should leave room for future additions. A wave
sequence of -1, 0, 1, 5 is better than -1, 0, 1, 2 because you can insert
at wave 3 without any renumbering.

**4. Waves control intent, not just API correctness**
The default ArgoCD ordering aims for API correctness — resources are created
without errors. Sync waves enforce intent — governance resources exist before
workloads start, matching your security and operational requirements.

**5. The annotation value must be a quoted string**
...
**6. The 2-second inter-wave delay is intentional — do not create unnecessary waves**
Each wave boundary adds ~2 seconds of overhead. Do not split independent
resources into separate waves just for organisational clarity. Group them in
the same wave.

**7. Watch for PVC WaitForFirstConsumer in early waves**
...
**8. Sync waves work within one Application — App-of-Apps for cross-app ordering**
If you need ordering across multiple Applications, use App-of-Apps (Demo-12)
with wave annotations on the child Application CRDs themselves.
